...
for het in range(num_het):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("../output/case" + str(ERSAA_case_num[het]) + "_ersaa/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/kNN/samp_" + str(ERSAA_samp_num[samp_size]+1) + "/ersaa_obj.txt") as ERSAA_knn_nohet_file:
...
for het in range(num_het):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			missing_data = []
			for data_rep in range(num_data_replicates):
				file_path = "../output/case" + str(ERSAA_case_num[het]) + "_ersaa/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/kNN/samp_" + str(ERSAA_samp_num[samp_size]+1) + "/ersaa_obj.txt"
